...
def clean_title(title, strip=None, strip_parentheses: bool = True) -> str:
    ...
    Booking systems often wrap the name with the building + instrument, e.g.
        "BLDG-A SEM-1 Alex Taylor (BLDG-A SEM-1)"
    ...
    examples above is "Alex Taylor" and "Jordan Lee".
    """
    s = title or ""
    ...
    # (the trailing "(BLDG-A SEM-1)" tells us what prefixes the title).
    paren_labels = []
...
def auto_strip_terms(titles: Sequence[Optional[str]], min_count: int = 2) -> list[str]:
    ...
    A booking system stamps the same "(BLDG-A SEM-2)" on every booking, while
    user ids like "( u1234567 )" are unique. Labels seen on >= min_count bookings
...
def classify_incident(
    ...
    incident_link_patterns: Sequence[str] = (),
) -> bool:
    """Decide whether an event from a booking feed is really an incident.

    An event counts as an incident if any of:
      - its CATEGORIES matches incident_categories,
      - an incident_keyword appears in its title,
      - an incident_link_pattern appears in its URL or description.

    The last is how many facility booking systems mark incidents - the event
    carries a link such as ".../inc/..." rather than a category. Matching is
    case-insensitive.
    """
    ...
    if any(k.lower() in title for k in incident_keywords):
        return True
    if incident_link_patterns:
        link = " ".join([
            booking.raw_properties.get("URL", ""),
            booking.raw_properties.get("DESCRIPTION", ""),
            booking.description or "",
        ]).lower()
        if any(p.lower() in link for p in incident_link_patterns):
            return True
    return False
...
def current_booking(bookings: Sequence[CalendarBooking], now: datetime):
    """The booking happening right now, or None."""
    for b in bookings:
        if is_active(b, now):
            return b
    return None
...
def next_booking(bookings: Sequence[CalendarBooking], now: datetime):
    """The soonest booking that starts after now, or None."""
    upcoming = []
    for b in bookings:
        if b.start is None:
            continue
        start = b.start if b.start.tzinfo else b.start.replace(tzinfo=now.tzinfo)
        if start > now:
...
def build_instrument_view(
    ...
    - booking_events: events from the instrument's booking feed (some may turn
      out to be incidents based on their categories/keywords/link).
    ...
    for ev in booking_events:
        if classify_incident(ev, incident_categories, incident_keywords, incident_link_patterns):
            incidents.append(ev)
        else:
            bookings.append(ev)
    ...
    todays_bookings = sorted(
        [
            b for b in bookings
            if b.start and (_to_tz(b.start, tz).date() == today or is_active(b, now))
        ],
