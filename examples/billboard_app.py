...
def load_instrument(inst, config, tz, now, start, end):
    ...
    if not inst.calendar_url and not inst.incident_url:
        view = InstrumentView(
            inst.equipment_id, inst.equipment_name, "nodata",
            error="No calendar_url configured",
        )
    ...
    try:
        ...
        # The feed is always classified the same way (categories / keywords /
        # link). incidents_only entries simply drop their tile in _serialize and
        # keep the detected incidents for the ticker.
...
def _summary_line(view, now, tz, mode):
    """A short headline for the tile, e.g. 'Free until 13:00' or 'In use until 15:00'."""
    if view.status == "down":
        return ""  # the incident reason is shown instead
    if view.status == "nodata":
        return "No data"
    cur = current_booking(view.bookings, now)
    nxt = next_booking(view.bookings, now)
    if mode != "full":
        # Public screens: no times, just availability.
        return "In use" if cur else "Available"
    if cur:
        return "In use until " + _fmt_hm(cur.end, tz) if cur.end else "In use"
    if nxt:
        return "Free until " + _fmt_hm(nxt.start, tz)
...
def _serialize(view, config, inst, tz, now, strip_terms):
    ...
    # At-a-glance summary line ("Free until 13:00" / "In use until 15:00").
    d["summary"] = _summary_line(view, now, tz, config.mode)
...
.panel-summary { font-size: 1.15vw; font-weight: 700; color: rgba(255,255,255,0.95);
                 margin-top: 0.5vw; }
...
/* The booking happening right now: solid white, dark text - "boarding now". */
.brow.now { background: #ffffff; color: #0b1020; font-weight: 800;
            box-shadow: 0 0 0 0.18vw rgba(255,255,255,0.65); }
.brow.now .bmeta { color: rgba(11,16,32,0.62); }
...
<div class="topbar">
  ...
  <div class="topright">
    ...
    <div style="text-align:right">
      ...
      <div class="datestr" id="updated"></div>
    </div>
  </div>
</div>
...
function panelHTML(inst) {
  ...
  if (st === 'down') {
    ...
    if (inst.summary) h += '<div class="panel-summary">' + esc(inst.summary) + '</div>';
...
function applyMeta(m) {
  ...
  if (m.generated_at) document.getElementById('updated').textContent = 'updated ' + m.generated_at;
...
def api_status():
    ...
    return jsonify({
        ...
        "meta": {
            ...
            "generated_at": datetime.now(tz).strftime("%H:%M:%S"),
        },
...
def _inspect_one(url):
    """Fetch one feed and guess an instrument entry from it."""
    try:
        if url.startswith("demo://"):
            data = make_demo_feed(url[len("demo://"):], datetime.now(timezone.utc))
        else:
            data = fetch_ical_from_url(url)
        cal = parse_ical_bytes(data)
        calname = str(cal.get("X-WR-CALNAME") or "").strip()
        bookings = parse_events_from_calendar(cal, "x", "x", url)
        # The label that recurs across bookings is usually the building+instrument
        labels = auto_strip_terms([b.title for b in bookings])
        label = labels[0] if labels else ""
        locations = [b.location for b in bookings if b.location]
        room = Counter(locations).most_common(1)[0][0] if locations else ""
        return {
            "calendar_url": url,
            "equipment_name": calname or label or "",
            "room": room,
            "event_count": len(bookings),
            "error": None,
        }
    except Exception as exc:
        return {"calendar_url": url, "equipment_name": "", "room": "",
                "event_count": 0, "error": str(exc)}


@app.route("/api/inspect", methods=["POST"])
def api_inspect():
    """Fetch a list of iCal URLs and return a suggested instrument per feed."""
    body = request.get_json(force=True, silent=True) or {}
    urls = [u.strip() for u in (body.get("urls") or []) if u and u.strip()]
    if not urls:
        return jsonify({"ok": True, "instruments": []})
    workers = max(1, min(8, len(urls)))
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as pool:
        results = list(pool.map(_inspect_one, urls))
    return jsonify({"ok": True, "instruments": results})
